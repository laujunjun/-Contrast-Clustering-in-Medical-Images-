...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(
        train_dataset,
        sampler=RandomSampler(train_dataset),   # 使用随机采样器
        batch_size=batch_size,
        drop_last=True, # 丢弃最后不完整的批次
        num_workers=int(args.workers),  # 设置工作线程数
        pin_memory=False,   # 不固定内存
        worker_init_fn=lambda id: np.random.seed(torch.initial_seed() // 2 ** 32 + id), # 初始化随机种子
        collate_fn=null_collate)    # 设置合并函数
    test_loader = DataLoader(
        test_dataset,
        sampler=SequentialSampler(test_dataset),    # 使用顺序采样器
        #batch_size=2,
        batch_size=batch_size,
        drop_last=False,
        num_workers=0,
    ...
    scaler = amp.GradScaler(enabled=is_amp) # 创建自动混合精度的缩放器
    net = model.Net(arg=args, resnet=res).to(device)    # 创建模型并转移到设备
    #res = ctranspath() #addnew
    #res.head = nn.Identity() #addnew
    #device = torch.device("cuda" if torch.cuda.is_available() else "cpu") #addnew
    #scaler = amp.GradScaler(enabled=is_amp) #addnew
    #net = model.Net(arg=args, resnet=res).to(device) #addnew
    ...
    for i in range(Epoch):
        for t, batch in enumerate(train_loader):
            if (t % (len(train_loader)-1) == 0 and t != 0): # 每个epoch的最后一个批次
                sum_train_loss = validate(net, test_loader, i)  # 验证模型
                Valid_nums += 1
                pass
            rate = get_learning_rate(optimizer) # 获取当前学习率
            batch_size = len(batch['index'])
            batch['image'] = batch['image'].cuda()
            batch['image_Argument'] = batch['image_Argument'].cuda()
            net.train()
            ...
            output = net(batch) # 前向传播
            loss_instance = output['loss_instance'].mean()  # 计算实例损失
            loss_cluster = output['loss_cluster'].mean()    # 计算聚类损失
            loss_barlow = output['loss_barlow'].mean()  # 总损失
            loss = (loss_instance + loss_cluster)*(1-args.alpha)+loss_barlow*args.alpha # 累加训练损失

            optimizer.zero_grad()
            lr = lr_schedule(iteration)
            optimizer.param_groups[0].update(lr=lr)
            scaler.scale(loss).backward()   # 反向传播
            scaler.unscale_(optimizer)  # 反向缩放
